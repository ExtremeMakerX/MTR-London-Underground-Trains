...
public class ModUpload {
	...
	public static void main(String[] args) throws IOException {
		if (args.length == 2) {
			...
			NetworkUtils.openConnectionSafeJson("https://www.minecrafttransitrailway.com/libs/latest/latest-lu-trains.json", jsonElement -> modVersion[0] = jsonElement.getAsJsonObject().get("version").getAsString());
			for (final String minecraftVersion : MINECRAFT_VERSIONS) {
				for (final ModLoader modLoader : ModLoader.values()) {
					...
					final String urlMtr = "https://www.minecrafttransitrailway.com/libs/latest/MTR-LU-Trains-" + modLoader.name + "-" + minecraftVersion + "-latest.jar";
					final String fileNameMtr = "MTR-LU-Trains-" + modVersionMtr + ".jar";
					...
					while (true) {
						...
						NetworkUtils.openConnectionSafe(urlMtr, inputStream -> success[0] = new ModId("357821", ModProvider.CURSE_FORGE).uploadFile("", modVersionMtr.toUpperCase(Locale.ENGLISH), "", dependenciesCurseForge, modLoader == ModLoader.FABRIC ? ReleaseStatus.RELEASE : ReleaseStatus.BETA, Collections.singleton(minecraftVersion), Collections.singleton(modLoader), false, inputStream, fileNameMtr, args[0]));
						if (success[0]) {
							break;
						}
					}
					...
					while (true) {
						final boolean[] success = {false};
						NetworkUtils.openConnectionSafe(urlMtr, inputStream -> success[0] = new ModId("", ModProvider.MODRINTH).uploadFile("The London Underground Trains", modVersionMtr.toUpperCase(Locale.ENGLISH), "", dependenciesModrinth, ReleaseStatus.RELEASE, Collections.singleton(minecraftVersion), Collections.singleton(modLoader), false, inputStream, fileNameMtr, args[1]));
						if (success[0]) {
							break;
						}
					}
